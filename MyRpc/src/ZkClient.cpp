...
{
    //加载zk的IP和端口号，默认为2181
    std::string host=MyRpcConfig::GetInstance().GetConfig("zookeeperip");
    std::string port=MyRpcConfig::GetInstance().GetConfig("zookeeperport");
...
std::string ZkClient::GetData(const char *path)
{
    ...
    {
        //获取成功
        // std::cout<<"获取zoodata成功"<<path<<std::endl;
        return buffer;
    }
